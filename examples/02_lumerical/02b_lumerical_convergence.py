#%%
"""
Comprehensive convergence study for Lumerical FDTD simulations using the fdtd_solver_lumerical class.
This script runs convergence studies for:
1. Mesh resolution (using Lumerical's discrete mesh levels 1-8)
2. Port width 
3. Port depth
...
def create_working_directory():
    ...
    base_dir = f"lumerical_convergence_study_{timestamp}"
    os.makedirs(base_dir, exist_ok=True)
    return base_dir

def run_mesh_convergence_study(base_working_dir):
    """Run mesh convergence study for different mesh resolutions using Lumerical's available mesh sizes."""
    
    # Lumerical available mesh sizes (cells per wavelength) - these get mapped internally to discrete levels
    mesh_sizes = [6, 10, 14, 18, 22, 26, 30, 34]
    ...
    # Load technology file - need to modify for Lumerical materials
    ...
    print(f"\n{'='*60}")
    print(f"LUMERICAL MESH CONVERGENCE STUDY")
    print(f"{'='*60}")
    ...
    for i, mesh_size in enumerate(mesh_sizes):
        print(f"\n--- Running simulation {i+1}/{len(mesh_sizes)} with mesh = {mesh_size} cells/λ ---")
        start_time = time.time()
        
        try:
            # Create Lumerical solver with current mesh size
            solver = fdtd_solver_lumerical(
                ...
                mesh=mesh_size,  # Current mesh size being tested (cells per wavelength)
...
def run_port_width_convergence_study(base_working_dir):
    ...
    for i, port_width in enumerate(port_widths):
        ...
        try:
            # Create Lumerical solver with current port width
            solver = fdtd_solver_lumerical(
                component=component,
                tech=technology,
                port_input=[component.ports[0]],  # Input port(s)
                ...
                wavelength_end=1.6,
                wavelength_points=50,  # Fewer points for faster simulation
                mesh=18,  # Fixed mesh size (good balance from convergence results)
                boundary=["PML", "PML", "PML"],
                symmetry=[0, 0, 0],
                z_min=-1.0,
                z_max=1.11,
...
def run_port_depth_convergence_study(base_working_dir):
    ...
    print(f"\n{'='*60}")
    print(f"LUMERICAL PORT DEPTH CONVERGENCE STUDY")
    print(f"{'='*60}")
    ...
    for i, port_depth in enumerate(port_depths):
        ...
        try:
            # Create Lumerical solver with current port depth
            solver = fdtd_solver_lumerical(
                component=component,
                tech=technology,
                port_input=[component.ports[0]],  # Input port(s)
...
def plot_mesh_convergence_results(results, base_working_dir):
    """Plot mesh convergence results for Lumerical."""
    ...
    for i, (size, s21) in enumerate(zip(valid_mesh_sizes, valid_s21_data)):
        ax1.plot(wavelengths, 10*np.log10(np.abs(s21)**2), 
                label=f'{size} cells/λ', linewidth=2)
    ax1.set_xlabel('Wavelength [μm]')
    ax1.set_ylabel('S21 Transmission [dB]')
    ax1.set_title('S21 vs Wavelength for Different Mesh Sizes (Lumerical)')
    ...
    for i, (size, s11) in enumerate(zip(valid_mesh_sizes, valid_s11_data)):
        ax2.plot(wavelengths, 10*np.log10(np.abs(s11)**2), 
                label=f'{size} cells/λ', linewidth=2)
    ax2.set_xlabel('Wavelength [μm]')
    ax2.set_ylabel('S11 Reflection [dB]')
    ax2.set_title('S11 vs Wavelength for Different Mesh Sizes (Lumerical)')
    ax2.grid(True, alpha=0.3)
    ax2.legend()
    
    # Plot 3: Convergence of S21 at target wavelength vs cells per wavelength
    ...
    ax3.set_xticks(valid_mesh_sizes)
    
    # Add secondary x-axis showing mesh levels (for reference)
    # Map mesh sizes back to approximate levels for display
    mesh_to_level = {6: 1, 10: 2, 14: 3, 18: 4, 22: 5, 26: 6, 30: 7, 34: 8}
    ax3_twin = ax3.twiny()
    ax3_twin.set_xlim(ax3.get_xlim())
    ax3_twin.set_xticks(valid_mesh_sizes)
    ax3_twin.set_xticklabels([f'L{mesh_to_level.get(size, "?")}' for size in valid_mesh_sizes])
    ax3_twin.set_xlabel('Mesh Level (reference)')
    ...
    # Plot 4: Convergence of S11 at target wavelength vs cells per wavelength
    ...
    ax4.set_xticks(valid_mesh_sizes)
    
    # Add secondary x-axis showing mesh levels (for reference)
    ax4_twin = ax4.twiny()
    ax4_twin.set_xlim(ax4.get_xlim())
    ax4_twin.set_xticks(valid_mesh_sizes)
    ax4_twin.set_xticklabels([f'L{mesh_to_level.get(size, "?")}' for size in valid_mesh_sizes])
    ax4_twin.set_xlabel('Mesh Level (reference)')
    ...
    # Save plot
    plot_path = os.path.join(base_working_dir, "lumerical_mesh_convergence_plot.png")
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"Lumerical mesh convergence plot saved to: {plot_path}")
    plt.show()
...
def plot_port_width_convergence_results(results, base_working_dir):
    ...
    ax1.set_xlabel('Wavelength [μm]')
    ax1.set_ylabel('S21 Transmission [dB]')
    ax1.set_title('S21 vs Wavelength for Different Port Widths (Lumerical)')
    ax1.grid(True, alpha=0.3)
    ax1.legend()
    ...
    ax2.set_xlabel('Wavelength [μm]')
    ax2.set_ylabel('S11 Reflection [dB]')
    ax2.set_title('S11 vs Wavelength for Different Port Widths (Lumerical)')
    ax2.grid(True, alpha=0.3)
    ax2.legend()
    ...
    ax3.set_ylabel(f'|S21| at {actual_wl:.3f} μm')
    ax3.set_title(f'S21 vs Port Width at λ = {actual_wl:.3f} μm (Lumerical)')
    ax3.grid(True, alpha=0.3)
    ...
    ax4.set_ylabel(f'|S11| at {actual_wl:.3f} μm')
    ax4.set_title(f'S11 vs Port Width at λ = {actual_wl:.3f} μm (Lumerical)')
    ax4.grid(True, alpha=0.3)
    ...
    # Save plot
    plot_path = os.path.join(base_working_dir, "lumerical_port_width_convergence_plot.png")
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"Lumerical port width convergence plot saved to: {plot_path}")
    plt.show()
    
    # Print convergence summary
    print(f"\n--- Lumerical Port Width Convergence Summary at λ = {actual_wl:.3f} μm ---")
...
def plot_port_depth_convergence_results(results, base_working_dir):
    """Plot port depth convergence results for Lumerical."""
    ...
    ax1.set_xlabel('Wavelength [μm]')
    ax1.set_ylabel('S21 Transmission [dB]')
    ax1.set_title('S21 vs Wavelength for Different Port Depths (Lumerical)')
    ax1.grid(True, alpha=0.3)
    ax1.legend()
    ...
    ax2.set_xlabel('Wavelength [μm]')
    ax2.set_ylabel('S11 Reflection [dB]')
    ax2.set_title('S11 vs Wavelength for Different Port Depths (Lumerical)')
    ax2.grid(True, alpha=0.3)
    ax2.legend()
    ...
    ax3.set_ylabel(f'|S21| at {actual_wl:.3f} μm')
    ax3.set_title(f'S21 vs Port Depth at λ = {actual_wl:.3f} μm (Lumerical)')
    ax3.grid(True, alpha=0.3)
    ...
    ax4.set_ylabel(f'|S11| at {actual_wl:.3f} μm')
    ax4.set_title(f'S11 vs Port Depth at λ = {actual_wl:.3f} μm (Lumerical)')
    ax4.grid(True, alpha=0.3)
    ...
    # Save plot
    plot_path = os.path.join(base_working_dir, "lumerical_port_depth_convergence_plot.png")
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"Lumerical port depth convergence plot saved to: {plot_path}")
    plt.show()
    
    # Print convergence summary
    print(f"\n--- Lumerical Port Depth Convergence Summary at λ = {actual_wl:.3f} μm ---")
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*80}")
    print(f"COMPREHENSIVE LUMERICAL CONVERGENCE STUDY")
    print(f"{'='*80}")
    ...
    # Save mesh results
    mesh_results_file = os.path.join(base_working_dir, "lumerical_mesh_convergence_results.pkl")
    with open(mesh_results_file, 'wb') as f:
        pickle.dump(mesh_results, f)
    print(f"Lumerical mesh results saved to {mesh_results_file}")
    
    # Plot mesh results
    plot_mesh_convergence_results(mesh_results, base_working_dir)
    print_timing_summary(mesh_results, "Lumerical Mesh Convergence")
    
    # 2. Port width convergence study
    width_results = run_port_width_convergence_study(base_working_dir)
    
    # Save width results
    width_results_file = os.path.join(base_working_dir, "lumerical_port_width_convergence_results.pkl")
    with open(width_results_file, 'wb') as f:
        pickle.dump(width_results, f)
    print(f"Lumerical port width results saved to {width_results_file}")
    
    # Plot width results
    plot_port_width_convergence_results(width_results, base_working_dir)
    print_timing_summary(width_results, "Lumerical Port Width Convergence")
    
    # 3. Port depth convergence study
    depth_results = run_port_depth_convergence_study(base_working_dir)
    
    # Save depth results
    depth_results_file = os.path.join(base_working_dir, "lumerical_port_depth_convergence_results.pkl")
    with open(depth_results_file, 'wb') as f:
        pickle.dump(depth_results, f)
    print(f"Lumerical port depth results saved to {depth_results_file}")
    
    # Plot depth results
    plot_port_depth_convergence_results(depth_results, base_working_dir)
    print_timing_summary(depth_results, "Lumerical Port Depth Convergence")
    
    # Final summary
    print(f"\n{'='*80}")
    print(f"LUMERICAL CONVERGENCE STUDY COMPLETE")
    print(f"{'='*80}")
    print(f"All results saved to: {base_working_dir}")
    print(f"Generated plots:")
    print(f"  - lumerical_mesh_convergence_plot.png")
    print(f"  - lumerical_port_width_convergence_plot.png") 
    print(f"  - lumerical_port_depth_convergence_plot.png")
    print(f"Generated data files:")
    print(f"  - lumerical_mesh_convergence_results.pkl")
    print(f"  - lumerical_port_width_convergence_results.pkl")
    print(f"  - lumerical_port_depth_convergence_results.pkl")
    
    # Print Lumerical mesh mapping reference
    print(f"\nLumerical Mesh Level Reference:")
    mesh_mapping = {1: 6, 2: 10, 3: 14, 4: 18, 5: 22, 6: 26, 7: 30, 8: 34}
    for level, cells in mesh_mapping.items():
        print(f"  Level {level}: {cells} cells per wavelength")
