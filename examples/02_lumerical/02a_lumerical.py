"""Crossing on Lumerical FDTD — the standard agnostic flow.

Every engine sets up the same way: get_solver(name)(component, tech, spec).
Requires a local Lumerical install + license; validate/build/estimate are
free and offline (build writes the complete .lsf setup script).
"""
...
if __name__ == "__main__":
    ...
    print("setup script:", solver.build().files["lsf"])
    print("estimate:", solver.estimate())

    # STEP 3+4: run (license seat), S-parameters, fields
    smatrix = solver.run()
