...
| 5 | 5.2c extraction.py | ✅ done (bidirectional mode projection a± = (∫E×Hm* ± ∫Em*×H)/(4Nm); exact identities tested: self-overlap=1, backward→(0,1), odd/even orthogonality, linearity of decompose; REAL tidy3d full-vector mode self-overlap = (1+0j) to 1e-9. **Phase 5.2 complete — full Tier-B pipeline: rasterize → modes → extraction**) | (next commit) |
| 5 | 5.4 fdtdz | ⏸ **DEFERRED (D9, 2026-07-08):** owner machine has no Nvidia GPU/nvcc and fdtdz 1.1.3 ships only a CUDA-building sdist — the package cannot even be imported here, and a fully-blind adapter is exactly the F10 bug class. Verified facts recorded for the future executor: kernel `fdtdz_jax.fdtdz(epsilon, dt, source_field, source_waveform, source_position, absorption_mask, pml_*, output_steps, use_reduced_precision)`; z-extent FIXED at 64−p (full) / 128−p (reduced precision); PML z-only, adiabatic absorbers x/y; real non-dispersive eps only; sources = y/z hyperplanes; deps jax≥0.4.16. The WP5.2 pipeline (rasterize→modes→extraction) is complete and tested — resume when GPU compute exists (7.x remote-compute lanes make this natural) | — |
...
| 6 | 6.4 Technology 2.0 | ✅ done (schema_version: 2 — top-level `materials:` named once, layers reference by name; expands into the v1 model BEFORE validation so equivalence holds by construction (tested: v2 fixture ≡ tech_unified.yaml legacy dict, all 3 engine paths validated on the v2 examples/tech.yaml incl. beamz indices from named materials); `gds-fdtd convert-tech` migrates v1→v2 with material dedupe + round-trip guard; docs/technology.rst v2-first; examples/tech.yaml is the reference v2 file. gdsfactory-LayerStack/KLayout-lyt interop remains future scope) | (next commit) |
| 6 | 6.2a–d docs · 6.2b citation · 6.3 v1.0 release | ⬜ not started (6.2 now UNBLOCKED: Pages, not RTD) | — |
...
| 7 | 7.5 cloud/licensed CI lanes | ✅ executor scope done: cloud-smoke.yml (dispatch-only, environment `cloud-tests`, budget-gated) + scripts/cloud_smoke.py **validated end-to-end for real** — estimates 0.025 FC/task via upload→estimate→delete, run PASSed all 4 physics checks, actual spend ≈0.05 FC; abort path verified exits 4 at BUDGET_FC=0.01 without spending. **FC LEDGER: +0.05 ⇒ ≈0.75 spent, ≈9.25 remain.** lumerical-nightly.yml merged in skipped state (guarded by repo var LUMERICAL_RUNNER) + docs/self_hosted_runner.md; SOLVER_STATUS.md with real last-verified dates linked from README. GPU lane (7.5.3) deferred with fdtdz (D9 — no CUDA anywhere yet). **Owner: create `cloud-tests` environment with required reviewer; set LUMERICAL_RUNNER var when a lab runner exists** | (next commit) |
...
| 7 | 7.3 JobSpec+CLI | ✅ done (execution/: JobSpec/JobResult/Budget + run_job; LocalBackend + SubprocessBackend (child = `python -m gds_fdtd.cli run`, hard wall-time from Budget); cli.py argparse with exit codes 0/2/3/4, `--import` for plugin registration, secrets env-only; 12 tests incl. round-trip law + REAL subprocess boundary (plugin registers in child, job_hash identical across backends) + budget refusal + .dat→.s2p/.npz/.h5 convert; docs/remote_compute.md SLURM/Modal/AWS-Batch reference snippets. **DEVIATION D10:** JobSpec references layout SOURCE (gds_path+top_cell+technology_path) instead of embedding Component — loaded components carry engine-resolved material objects (deliberate Phase-2 choice) that don't belong in a shipped payload; reconstruction through the standard loading path makes the round-trip law exact. Budget.max_flexcredits is refusal-only (=0 blocks non-free engines) until estimate() reports numeric cost) | (next commit) |
| 7 | 7.4 containers/typing | ◐ partial: item 1 OS×py matrix ✅ (done at WP0.2); item 4 ✅ — py.typed shipped, `mypy --strict` CLEAN on the typed public core (geometry, technology, smatrix, spec, execution/) incl. fixing a real annotation bug (load_cell claimed `-> pya.Cell`, returns a tuple); CI typecheck job is now REQUIRED (strict core hard-fails, package-wide advisory step remains while legacy modules burn down; D4 TODO retired). Items 2 (ghcr images) + 3 (conda-forge) ⏸ deferred: no container runtime on this machine (D9 principle — don't ship what can't be validated) and feedstock submission is owner-level | (next commit) |
| 7 | 7.6 polish | ◐ items 1/3/4 done: errors.py hierarchy (GdsFdtdError tree, DUAL-inherits the builtin each site historically raised — zero caller breakage, tested) swept through solvers+execution; GdsFdtdSettings (pydantic-settings, GDS_FDTD_ env prefix: cache_dir/default_budget_fc/log_level/log_format/telemetry-OFF) wired into run_cached() default + logging; GDS_FDTD_LOG_FORMAT=json emits JSON-lines (tested). Item 2 (tenacity retries on cloud calls) ⏸ deferred: touching validated money-spending paths needs its own live-revalidation session (F10 discipline); item 5 is WP6.2-gated | (next commit) |
...
| WP0.4 | ✅ done | `a3aac21` + fixup | AST import check; 6 xfails (broken legacy examples), tidy3d-importing ones skip in base profile / run in all-extras leg. **Finding F2 (fixed in fixup commit):** test_lyprocessor.py installed fake klayout/prefab/simprocessor modules into sys.modules at import time, poisoning any test collected after it (golden tests got a fake simprocessor). Rewritten with monkeypatch-scoped fakes. **Executor lesson: `pytest \| tail` hides the exit code — always assert `$?` from pytest itself.** |
| **Phase 0** | **✅ COMPLETE** | — | |
| WP1.1 | ✅ done | `317ede5` | dilate_1d rewritten; descending-order + xy-both-corners tests; "xy" test expectation updated (old code failed to extend x1) |
| WP1.3 | ✅ done | `985fa8d` | trailing-digit port.idx (raises on digitless names); sparams gains `_number_of` + parsed `*_num` properties; `S()` compares integers; test_core expectations updated per card |
| WP1.4 | ✅ done | `b63ab94` | get_material ValueErrors name the offending key; load_region iterates all DevRec shapes, raises on empty layer, union-bbox on multiple. NOTE: old code applied `itrans()` only to an unused var — untransformed behavior preserved deliberately (goldens) |
| WP1.2 | ✅ done | `4ecb85a` | port_input=None ⇒ all ports; normalization+validation in __init__; `_get_active_ports` now trivial; all mutable defaults (solver, fdtd_port, core.structure) fixed; tests/test_solver.py instantiates the base class directly (its @abstractmethods lack ABCMeta) |
| WP1.7 | ✅ done | `78672c6` | package-logger-only setup_logging (propagate=False, closes own handlers); NullHandler in __init__ |
| WP1.8 | ✅ done | `4654c4d` | load_device was dead (called load_component_from_tech with nonexistent `ly=` kwarg); now returns component, writes to output_dir (tempdir default), never mutates input; apply_prefab(gds_in, gds_out, top_cell, model=) refuses same-path |
| WP1.6 | ✅ done | `76154b5` | angle² lines removed; `write_dat()` replaces s_parameter_writer (entry-driven headers, own freq vector, full-precision floats, magnitude not power); round-trip test with partial out-of-order 2×2-mode matrix |
| WP1.5+1.9 | ✅ done | `b235df3` | boundary honored (PML/Metal→PEC/Periodic per axis), run_time via shared helper, background+monitors centered on component, dead field plumbing deleted; offline test suite builds full setup incl. ComponentModeler with zero network. **Finding F3: tidy3d 2.8.5 requires `pytz` but doesn't declare it** — installed manually in .venv; revisit in WP4.1 |
| **Phase 1** | **✅ COMPLETE** | — | All 19 audited bugs that were Phase-1-scoped (B1,B5–B17 core set) fixed with regression tests. Goldens unchanged throughout. |
...
| **Phase 2** | **✅ COMPLETE** | — | |
| WP3.1a | ✅ done | `7a14522` | SimulationSpec (defaults identical to legacy kwargs); fdtd_solver mirrors spec into legacy list attrs; `_validate_simulation_parameters` re-derives spec at setup. Tightening (documented): invalid params now fail at construction |
| WP3.1b | ✅ done | `d95f346` | `solvers/base.py`: Solver ABC (validate/build/estimate/run), SolverCapabilities, SetupArtifacts, ResourceEstimate, injection_plan()/domain()/frequencies_hz()/describe(), in-package registry. `tests/conformance/` suite parametrized over the registry (purity via socket+cwd monkeypatch, offline deterministic build, etc.); FakeSolver = reference impl (reciprocal passive SMatrix) — also the workhorse for WP5.5/WP7.3 tests |
| **Phase 3 remaining — handoff notes for the next session** | | | Suite at handoff: **161 passed, 6 xfailed** (legacy examples, die in WP6.1). Coverage 57%. **WP3.1c** (port tidy3d): create `solvers/tidy3d.py::Tidy3DSolver(Solver)` — move the guts of `solver_tidy3d.fdtd_solver_tidy3d` (`_create_structures/_create_boundary_spec/_create_smatrix_ports/_create_field_monitor`) into build() returning SetupArtifacts(native=ComponentModeler, files={gds: ...}); run() = modeler.run() → `_convert_smatrix_to_sparameters` logic reshaped to return SMatrix directly (use `SMatrix.from_entries`); keep the legacy class as a thin wrapper calling the new one (its tests in test_solver_tidy3d.py must keep passing unchanged); register with `@register_solver`, `probe_available()` checks tidy3d import; conformance suite must go green for it (`build()` needs the GDS export moved OUT of setup into build(), written to `self.workdir` only — constructor purity is enforced by the suite). **WP3.1d** (port lumerical): same pattern; CRITICAL: move the module-level `from lumapi import FDTD` import inside methods; build() produces the .lsf transcript via a recorded-eval shim; probe_available reports lumapi absence. **WP3.1e**: `[project.entry-points."gds_fdtd.solvers"]` + wire external discovery into available_solvers(); old-constructor DeprecationWarnings; kill the remaining print-summaries in favor of describe(). Then WP3.2 (lumerical hardening). Env note: `.venv` has tidy3d 2.8.5 (+pytz manually), h5py, scikit-rf installed |
